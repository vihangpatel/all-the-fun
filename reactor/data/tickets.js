export default {
	tickets: [
		{
			type: "Conference Only Pass",
			deliverables: [
				{
					text: "ACCESS TO 2 FULL DAY CONFERENCE",
					checkbox: true,
				},
				{
					text: "Lunch - Tea / Coffee",
					checkbox: true,
				},
				{
					text: "5 Stay Hotel Occupancy for 3 Nights",
				},
				{
					text: "Access to Networking After Party",
				},
				{
					text: "Complementary React India Tshirt and merchandise",
				},
				{
					text: "Any 1 Workshop Access",
				},
			],
			prices: [
				{
					text: "INDIAN ATTENDEE (INR)",
					value: "9999 INR",
				},
				{
					text: "INTERNATIONAL ATTENDEE ( EUR)",
					value: "175 EUR",
				},
			],
			buyticketUrl: "https://www.townscript.com/e/react-india-2019/booking",
		},
		{
			type: "WORKSHOP ONLY PASS",
			deliverables: [
				{
					text: "ACCESS TO 2 FULL DAY CONFERENCE",
				},
				{
					text: "Lunch - Tea / Coffee",
					checkbox: true,
				},
				{
					text: "5 Stay Hotel Occupancy for 3 Nights",
				},
				{
					text: "Access to Networking After Party",
				},
				{
					text: "Complementary React India Tshirt and merchandise",
				},
				{
					text: "Any 1 Workshop Access",
					checkbox: true,
				},
			],
			prices: [
				{
					text: "INDIAN ATTENDEE (INR)",
					value: "9999 INR",
				},
				{
					text: "INTERNATIONAL ATTENDEE ( EUR)",
					value: "175 EUR",
				},
			],
			buyticketUrl: "https://www.townscript.com/e/react-india-2019/booking",
		},
		{
			type: "REGULAR CONFERENCE PASS",
			deliverables: [
				{
					text: "ACCESS TO 2 FULL DAY CONFERENCE",
					checkbox: true,
				},
				{
					text: "Lunch - Snacks -Tea / Coffee",
					checkbox: true,
				},
				{
					text: "5 Stay Hotel Occupancy for 3 Nights",
				},
				{
					text: "Access to Networking After Party",
					checkbox: true,
				},
				{
					text: "Complementary React India Tshirt and merchandise",
					checkbox: true,
				},
				{
					text: "Any 1 Workshop Access",
				},
			],
			prices: [
				{
					text: "INDIAN ATTENDEE (INR)",
					value: "18961 INR",
				},
				{
					text: "INTERNATIONAL ATTENDEE ( EUR)",
					value: "250 EUR",
				},
			],
			buyticketUrl: "https://www.townscript.com/e/react-india-2019/booking",
		},
		{
			type: "CONFERENCE + ACCOMODATION PASS (SHARED)",
			deliverables: [
				{
					text: "ACCESS TO 2 FULL DAY CONFERENCE",
					checkbox: true,
				},
				{
					text: "Breakfast - Lunch - Dinner",
					checkbox: true,
				},
				{
					text: "	5 Stay Hotel Shared Occupancy for 3 Nights",
				},
				{
					text: "Access to Networking After Party",
					checkbox: true,
				},
				{
					text: "Complementary React India Tshirt and merchandise",
					checkbox: true,
				},
				{
					text: "Any 1 Workshop Access",
				},
			],
			prices: [
				{
					text: "INDIAN ATTENDEE (INR)",
					value: "22201 INR",
				},
				{
					text: "INTERNATIONAL ATTENDEE ( EUR)",
					value: "275 EUR",
				},
			],
			note: `The ticket includes Shared occupancy, 
			so you would have to share your room with 
			some same gender tech buddy.`,
			buyticketUrl: "https://www.townscript.com/e/react-india-2019/booking",
		},
		{
			type: "CONFERENCE + ACCOMODATION PASS (SINGLE)",
			deliverables: [
				{
					text: "ACCESS TO 2 FULL DAY CONFERENCE",
					checkbox: true,
				},
				{
					text: "Breakfast - Lunch - Dinner",
					checkbox: true,
				},
				{
					text: "5 Stay Hotel Shared Occupancy for 3 Nights",
				},
				{
					text: "Access to Networking After Party",
					checkbox: true,
				},
				{
					text: "Complementary React India Tshirt and merchandise",
					checkbox: true,
				},
				{
					text: "Any 1 Workshop Access",
				},
			],
			prices: [
				{
					text: "INDIAN ATTENDEE (INR)",
					value: "25423 INR",
				},
				{
					text: "INTERNATIONAL ATTENDEE ( EUR)",
					value: "315 EUR",
				},
			],
			note: `The ticket includes Single occupancy(Entire Room),
			You can invite one guest with you for whom 
			we provide food too.Although to get him/her 
			conference access you will need to buy
			conference pass.`,
			buyticketUrl: "https://www.townscript.com/e/react-india-2019/booking",
		},
		{
			type: "FULL ACCESS PASS",
			deliverables: [
				{
					text: "ACCESS TO 2 FULL DAY CONFERENCE",
					checkbox: true,
				},
				{
					text: "Breakfast - Lunch - Dinner",
					checkbox: true,
				},
				{
					text: "	5 Stay Hotel Shared Occupancy for 3 Nights",
				},
				{
					text: "Access to Networking After Party",
					checkbox: true,
				},
				{
					text: "Complementary React India Tshirt and merchandise",
					checkbox: true,
				},
				{
					text: "Any 1 Workshop Access",
				},
			],
			prices: [
				{
					text: "INDIAN ATTENDEE (INR)",
					value: "33898 INR",
				},
				{
					text: "INTERNATIONAL ATTENDEE ( EUR)",
					value: "475 EUR",
				},
			],
			buyticketUrl: "https://www.townscript.com/e/react-india-2019/booking",
		},
		{
			type: "DIVERSITY SUPPORTER (FULL ACCESS PASS)",
			deliverables: [
				{
					text: "ACCESS TO 2 FULL DAY CONFERENCE",
					checkbox: true,
				},
				{
					text: "Breakfast - Lunch - Dinner",
					checkbox: true,
				},
				{
					text: "	5 Stay Hotel Shared Occupancy for 3 Nights",
				},
				{
					text: "Access to Networking After Party",
					checkbox: true,
				},
				{
					text: "Complementary React India Tshirt and merchandise",
					checkbox: true,
				},
				{
					text: "Any 1 Workshop Access",
				},
			],
			prices: [
				{
					text: "INDIAN ATTENDEE (INR)",
					value: "22201 INR",
				},
				{
					text: "INTERNATIONAL ATTENDEE ( EUR)",
					value: "275 EUR",
				},
			],
			note: `With Diversity Supported tickets, you purchase a 
			FULL ACCESS ticket for yourself and 
			help someone of our scholarship program 
			to attend the conference by paying a part of their ticket. 
			Your support can make a difference!"`,
			buyticketUrl: "https://www.townscript.com/e/react-india-2019/booking",
		},
	],
}
