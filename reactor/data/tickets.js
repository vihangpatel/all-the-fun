export default {
  tickets: [
    {
      type: 'Conference Only Pass',
      subheader: 'Limited tickets only',
      deliverables: [
        {
          text: 'ACCESS TO THE FULL TWO DAY CONFERENCE',
          checkbox: true
        },
        {
          text: 'Lunch - Tea / Coffee',
          checkbox: true
        },
        {
          text: 'Access to Networking After Party on day 2'
        },
        {
          text: 'Complementary React India Tshirt'
        },
        {
          text: '5 Star Hotel Occupancy for 3 Nights'
        },
        {
          text: 'Any 1 Workshop Access'
        }
      ],
      prices: [
        {
          text: 'INDIAN ATTENDEE',
          value: '9999 INR'
        },
        {
          text: 'INTERNATIONAL ATTENDEE',
          value: '200 EUR'
        }
      ],
      buyticketUrl: 'https://www.townscript.com/e/react-india-2019/booking',
      disabled: true
    },
    {
      type: 'REGULAR CONFERENCE PASS',
      deliverables: [
        {
          text: 'ACCESS TO THE FULL TWO DAY CONFERENCE',
          checkbox: true
        },
        {
          text: 'Lunch - Tea / Coffee',
          checkbox: true
        },
        {
          text: 'Access to Networking After Party on day 2',
          checkbox: true
        },
        {
          text: 'Complementary React India Tshirt',
          checkbox: true
        },
        {
          text: '5 Star Hotel Occupancy for 3 Nights'
        },
        {
          text: 'Any 1 Workshop Access'
        }
      ],
      prices: [
        {
          text: 'INDIAN ATTENDEE',
          value: '18961 INR'
        },
        {
          text: 'INTERNATIONAL ATTENDEE',
          value: '250 EUR'
        }
      ],
      buyticketUrl: 'https://www.townscript.com/e/react-india-2019/booking',
      disabled: false
    },
    {
      type: 'WORKSHOP PASS',
      deliverables: [
        {
          text: 'Any 1 Workshop Access',
          checkbox: true
        },
        {
          text: 'Lunch - Tea / Coffee',
          checkbox: true
        },
        {
          text: 'Advanced training from industry experts',
          checkbox: true
        },
        {
          text: 'Small size group for maximum individual reach',
          checkbox: true
        },
        {
          text: 'Hands-on practical sessions',
          checkbox: true
        },
        {
          text: 'ACCESS TO THE FULL TWO DAY CONFERENCE'
        }
      ],
      prices: [
        {
          text: 'INDIAN ATTENDEE',
          value: '9999 INR'
        },
        {
          text: 'INTERNATIONAL ATTENDEE',
          value: '200 EUR'
        }
      ],
      buyticketUrl: 'https://www.townscript.com/e/react-india-2019/booking',
      disabled: false
    },
    {
      type: 'CONFERENCE + ACCOMODATION PASS (SHARED)',
      deliverables: [
        {
          text: 'ACCESS TO THE FULL TWO DAY CONFERENCE',
          checkbox: true
        },
        {
          text: 'Breakfast - Lunch - Dinner',
          checkbox: true
        },
        {
          text: '5 Star Hotel Shared Occupancy for 3 Nights (26th-29th September)',
          checkbox: true
        },
        {
          text: 'Stay will be at Kenilworth Beach Resort & Spa',
          checkbox: true
        },
        {
          text: 'Shared room with same gender developer friend',
          checkbox: true
        },
        {
          text: 'Access to Networking After Party on day 2',
          checkbox: true
        },
        {
          text: 'Complementary React India Tshirt',
          checkbox: true
        },
        {
          text: 'Any 1 Workshop Access'
        }
      ],
      prices: [
        {
          text: 'INDIAN ATTENDEE',
          value: '22201 INR'
        },
        {
          text: 'INTERNATIONAL ATTENDEE',
          value: '275 EUR'
        }
      ],
      note: `The ticket includes Shared occupancy, 
			so you would have to share your room with 
			some same gender tech buddy.`,
      buyticketUrl: 'https://www.townscript.com/e/react-india-2019/booking',
      disabled: false
    },
    {
      type: 'CONFERENCE + ACCOMODATION PASS (SINGLE)',
      deliverables: [
        {
          text: 'ACCESS TO THE FULL TWO DAY CONFERENCE (1 PASS ONLY)',
          checkbox: true
        },
        {
          text: 'Breakfast - Lunch - Dinner',
          checkbox: true
        },
        {
          text: 'Access to Networking After Party on day 2',
          checkbox: true
        },
        {
          text: 'Complementary React India Tshirt',
          checkbox: true
        },
        {
          text: '5 Star Hotel Single Occupancy for 3 Nights (26th-29th September)',
          checkbox: true
        },
        {
          text: 'Stay will be at Kenilworth Beach Resort & Spa',
          checkbox: true
        },
        {
          text: 'Option to invite 1 guest for stay (Conference ticket not included for the guest)',
          checkbox: true
        },
        {
          text: 'Guest Food Pass included',
          checkbox: true
        },
        {
          text: 'Any 1 Workshop Access'
        }
      ],
      prices: [
        {
          text: 'INDIAN ATTENDEE',
          value: '25423 INR'
        },
        {
          text: 'INTERNATIONAL ATTENDEE',
          value: '315 EUR'
        }
      ],
      note: `The ticket includes Single occupancy(Entire Room),
			You can invite one guest with you for whom 
			we provide food too.Although to get him/her 
			conference access you will need to buy
			conference pass.`,
      buyticketUrl: 'https://www.townscript.com/e/react-india-2019/booking',
      disabled: false
    },
    {
      type: 'FULL ACCESS PASS',
      deliverables: [
        {
          text: 'ACCESS TO THE FULL TWO DAY CONFERENCE(1 PASS ONLY)',
          checkbox: true
        },
        {
          text: 'Breakfast - Lunch - Dinner',
          checkbox: true
        },
        {
          text: 'Access to Networking After Party on day 2',
          checkbox: true
        },
        {
          text: 'Complementary React India Tshirt',
          checkbox: true
        },
        {
          text: '5 Star Hotel Single Occupancy for 4 Nights (25th-29th September)',
          checkbox: true
        },
        {
          text: 'Stay at Planet Hollywood Beach Resort from 25th - 26th September',
          checkbox: true
        },
        {
          text: 'Stay at Kenilworth Beach Resort & Spa from 26th - 29th September',
          checkbox: true
        },
        {
          text: 'Option to invite 1 guest for stay (Conference ticket not included for the guest)',
          checkbox: true
        },
        {
          text: 'Guest Food Pass included',
          checkbox: true
        },
        {
          text: 'Any 1 Workshop Access',
          checkbox: true
        }
      ],
      prices: [
        {
          text: 'INDIAN ATTENDEE',
          value: '33898 INR'
        },
        {
          text: 'INTERNATIONAL ATTENDEE',
          value: '475 EUR'
        }
      ],
      buyticketUrl: 'https://www.townscript.com/e/react-india-2019/booking'
    },
    {
      type: 'DIVERSITY SUPPORTER (FULL ACCESS PASS)',
      deliverables: [
        {
          text: 'ACCESS TO THE FULL TWO DAY CONFERENCE',
          checkbox: true
        },
        {
          text: 'Breakfast - Lunch - Dinner',
          checkbox: true
        },
        {
          text: 'Access to Networking After Party on day 2',
          checkbox: true
        },
        {
          text: 'Complementary React India Tshirt',
          checkbox: true
        },
        {
          text: '5 Star Hotel Single Occupancy for 4 Nights',
          checkbox: true
        },
        {
          text: 'Stay at Planet Hollywood Beach Resort from 25th - 26th September',
          checkbox: true
        },
        {
          text: 'Stay at Kenilworth Beach Resort & Spa from 26th - 29th September',
          checkbox: true
        },
        {
          text: 'Any 1 Workshop Access',
          checkbox: true
        },
        {
          text: 'FULL ACCESS ticket for yourself',
          checkbox: true
        },
        {
          text: 'Help someone of our scholarship program by paying a 50% of their ticket',
          checkbox: true
        }
      ],
      prices: [
        {
          text: 'INDIAN ATTENDEE',
          value: '59322 INR'
        },
        {
          text: 'INTERNATIONAL ATTENDEE',
          value: '735 EUR'
        }
      ],
      buyticketUrl: 'https://www.townscript.com/e/react-india-2019/booking',
      disabled: false
    }
  ]
}
